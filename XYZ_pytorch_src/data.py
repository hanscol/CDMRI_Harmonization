...
class dataset(Dataset):
    def __init__(self, inputDir, targetDir):
    ...
    def load_image(self):
        ...
        self.target_img = [nib.load(target_file[0]).get_fdata(), nib.load(target_file[1]).get_fdata()]
        self.xyz_map = np.zeros(np.append(self.input_img[0].shape[0:3], 3))

        for y in range(self.xyz_map.shape[0]):
            for x in range(self.xyz_map.shape[1]):
                for z in range(self.xyz_map.shape[2]):
                    self.xyz_map[y, x, z, :] = [y / self.xyz_map.shape[0], x / self.xyz_map.shape[1], z / self.xyz_map.shape[2]]
    ...
    def get_patch(self):
        ...
        patch[pymin:pymax, pxmin:pxmax, pzmin:pzmax, 30:] = self.xyz_map[ymin:ymax, xmin:xmax, zmin:zmax, :]
